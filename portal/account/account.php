...
if ($action == 'verify_email') {
    if (!empty($_SESSION['verifyPortalEmail']) && ($_SESSION['verifyPortalEmail'] === true)) {
        if (!empty($GLOBALS['portal_onsite_two_register']) && !empty($GLOBALS['google_recaptcha_site_key']) && !empty($GLOBALS['google_recaptcha_secret_key'])) {
            ...
            if (!$recaptcha) {
                echo xlt("Algo salíó mal. Sugiero contactar a hola@epa-bienestar.com.");
                cleanupRegistrationSession();
                exit;
            }
            // process
            ...
            if ($rtn) {
                Header::setupHeader();
                echo '<div class="alert alert-success" role="alert">' . xlt("Verifique su correo (Posiblemente también en SPAM) Para más instrucciones del proceso de registro. Si no recibe un mail contactar a hola@epa-bienestar.com.") . '</div>';
            } else {
                echo xlt("Algo salió mal. Sugiero contactar a hola@epa-bienestar.com.");
            }
        }
    }
    cleanupRegistrationSession();
    ...
    if (($_SESSION['register_silo_ajax'] ?? null) === true && ($_SESSION['register'] ?? null) === true && isset($_SESSION['pid'])) {
        if (!empty($GLOBALS['portal_onsite_two_register']) && !empty($GLOBALS['google_recaptcha_site_key']) && !empty($GLOBALS['google_recaptcha_secret_key'])) {
            ...
            if ($rtn) {
                ...
                Header::setupHeader();
                echo '<div class="alert alert-success" role="alert">' . xlt("Sus nuevas credenciales de identificación fueron enviadas. Verifique su mail y/o SPAM. Una vez que ingrese al portal puede contactarnos en el chat! Esperamos su visita pronto!") . '</div>';
            } else {
                (new SystemLogger())->debug("account.php action do_signup apparently not successful");
                Header::setupHeader();
                echo '<div class="alert alert-danger" role="alert">' . xlt("Problemas con el registro. Sugiero contactar a hola@epa-bienestar.com para asistencia!") . '</div>';
            }
        } else {
