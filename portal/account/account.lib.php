...
function notifyAdmin($pid, $provider): void
{
    ...
    $note = xlt("Nuevo paciente en el portal. Por favor! Recordar chequear nuevos turnos");
    $title = xlt("Nuevo Paciente");
...
function verifyEmail(string $languageChoice, string $fname, string $mname, string $lname, string $dob, string $email): bool
{
    ...
    if ($emailPrepSend) {
        ...
        $mail->Subject = xlt('Verifique su correo para Registrarse en el Portal de Pacientes');
...
function doCredentials($pid, $resetPass = false, $resetPassEmail = ''): bool
{
    ...
    $email_subject = xlt('Acceso al Portal de Pacientes') . ' / ' . xlt('3rd Party API Access');
    $email_sender = $GLOBALS['patient_reminder_sender_email'];
    $mail->AddReplyTo($email_sender, $email_sender);
    $mail->SetFrom($email_sender, $email_sender);
    ...
    if ($mail->Send()) {
        ...
        if ($resetPass) {
            ...
            $title = xlt("Fallo en el Registro");
            $admin_msg = "\n" . xlt("No se puedo enviar las credenciales luego de la registración al Portal del Pacientes.");
            $admin_msg .= "\n" . "EMAIL ERROR: " . $email_status;
            $admin_msg .= "\n" . xlt("Por favor seguir.");
            // send note
